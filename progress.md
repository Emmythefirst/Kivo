...
**GitHub:** the project had never been a git repo before now (confirmed back in Session 1). Ran `git init`, reviewed `git status` after `git add -A` to confirm nothing unexpected was staged (no `.env`, no `node_modules/`, no `android/` build output — the existing `.gitignore` from Session 1 did its job correctly on the first real commit), double-checked `.env.example` had only a redacted placeholder key, then committed and pushed to the user's pre-created repo at `github.com/emmythefirst/kivo`. GitHub responded with a "this repository moved" notice — the actual repo is case-normalized to `github.com/Emmythefirst/Kivo` — the push succeeded via redirect regardless, and the local `origin` remote was updated to the canonical URL afterward so future pushes don't depend on the redirect. This closes the GitHub-repo requirement for hackathon submission.

**Vercel:** walked the user through hosting `web/` for real, step by step, since this was their first time using Vercel:
- `npm i -g vercel`, then `vercel login` (device-code browser flow — a scary-looking `EPIPE`/worker-timeout stack trace printed alongside the login prompt, but this is just noise from the CLI's background update-checker and didn't affect the actual login, which succeeded).
- `cd web && vercel --prod`, walked through the first-deploy prompts live via screenshots: "Create a new project" (not "Search all projects", since no project existed yet), accepted the default name, accepted the default code directory (`./`), and accepted "no" to customizing build settings (correct default here — `web/` has no `package.json` and no build step, so Vercel correctly serves it as a static site).
- One prompt went the opposite of what was recommended: "Connect this Git repository to automatically deploy changes on every push?" was answered "yes" (the plan had suggested "no" for a simpler first pass) — harmless either way, it just means this page will also auto-redeploy whenever `web/` changes get pushed to GitHub, which is a convenience, not a problem.
- The GitHub-connection step itself then failed with "Failed to connect Emmythefirst/Kivo to project" — but this didn't block the actual deployment, which completed successfully regardless (`✓ Ready in 5s`). Net effect: the page is live, just without the auto-redeploy-on-push convenience; deploying future changes just means re-running `vercel --prod` manually from `web/`.
- **Verified live**, not just trusted the CLI output: `curl`'d the production URL directly, confirmed HTTP 200 and the real `index.html` content (checked for the `<title>Kivo — Payment Request</title>` tag specifically, not just "some page loaded").

**Live fallback page URL:** `https://web-rouge-one-97.vercel.app` — same query-string-swap testing method as the localhost round applies here (take the `?i=...&to=...&...` portion of any real Kivo-generated link and append it to this URL). Not yet re-tested with an actual live payment against this specific URL (only against `localhost:3000` previously) — worth doing once, though the underlying page code is identical to what was already verified working, so this is a hosting-path check, not expected to surface a new bug.

### 2026-09-21 — Session 3 continued: full visual redesign applied from a shared Claude design artifact

User designed a new UI in Claude's design/canvas tool and shared it for exact implementation across the mobile app (explicitly: "remove the mockup data but apply the exact design").

**Decoding the design.** The share link (`claude.ai/design/p/...`) wasn't a plain artifact URL the `Artifact` tool could read directly — needed the underlying `claude.ai/artifact/<id>` link instead. Even then, `action: "read"` returned a compiled *bundler wrapper* page (gzip+base64 asset manifest meant to be unpacked client-side in a browser), not readable source. Wrote a one-off Node script (`decode-artifact.mjs` in the session scratchpad) to base64-decode + gunzip every manifest entry directly, recovering: the actual page template (a declarative pseudo-JSX DSL — `sc-if`/`sc-for`/`x-import` tags — encoding every screen's exact layout/colors/spacing) and a `text/jsx` component file with the full mock state machine, seed data, and event handlers. This is the only way the real design (colors, fonts, exact copy, layout) was recoverable — reading the rendered artifact alone would not have surfaced it.

**Design tokens extracted:** near-black `#0A0A0D` background, `#15151B`/`#13131A` surfaces, lime `#C6F24E` as the single money/CTA accent, purple `#9E8CFC` reserved for wallet/Solana-branding moments, `#FF6B5E` danger, Manrope (body, weights 400–800) + Space Grotesk (headings/amounts, weights 500–700) fonts, large border radii (14–26px) throughout.

**Rewrote `src/theme/index.ts` entirely** — new `color`/`radius`/`space` tokens, a `font` map of the loaded Google Font family names, restructured `type` text styles, plus new shared helpers this redesign needed: `initials()`, `avatarColor()` (deterministic per-name/id color from a fixed palette, matching the design's avatar circles), and `timeAgo()` (relative-time labels like "2h ago" for request rows, which the design surfaces but the app never previously displayed).

**New dependencies, both required real installs + native rebuilds, not just npm installs:**
- `expo-font`, `@expo-google-fonts/manrope`, `@expo-google-fonts/space-grotesk` — for the two brand fonts. Installed via `npx expo install` (not manual version pinning) so Expo resolves SDK-52-compatible versions itself.
- `expo-file-system` — **a real missing dependency this surfaced, not a design-side bug.** First rebuild after adding the fonts failed at runtime with `Font load failed: ... Module 'expo.modules.interfaces.filesystem.AppDirectories' not found` — `expo-font`'s `useFonts` uses `expo-asset`'s `downloadAsync()` internally, which on Android requires `expo-file-system` to supply that interface module. Nothing in the app before this ever exercised that code path (no runtime asset-downloading previously happened), so the gap existed silently until font loading was the first thing to hit it. Fixed by installing `expo-file-system` and rebuilding again.
- Added a `[loaded, error]` destructure (previously just `[loaded]`) on both `useFonts` calls in `App.tsx`, surfacing the load error directly on-screen instead of leaving a font-load failure indistinguishable from "still loading" forever — this is exactly what caught the `expo-file-system` gap above in one screenshot instead of a guessing round.

**Rebuilt every screen to match the design exactly, wired to real data (not the mock's fake state):**
- `ConnectScreen` — two-square logo mark, wordmark, devnet/MWA pill, lime CTA. Connecting-overlay state now maps to the real `connecting` boolean from `useWallet()` (the design's mock had its own fake `setTimeout`-based connect simulation — replaced with the actual MWA flow, unchanged underneath).
- `HomeScreen` — pending-total card, avatar-circle request rows, per-row paid/pending status and bill progress bars. The design's mock never checked real paid status for rows at all (just displayed everything as "pending" indefinitely); added real `isRequestMarkedPaid()` checks for every single request and every bill participant so the new progress bars and status labels reflect actual on-chain state — required *because* the instruction was to remove mockup data, not just remove mock text.
- `NewRequestScreen` (composer) — the biggest behavioral change: replaced the old plain decimal `TextInput` amount field with the design's tap-a-numeric-keypad amount entry (digit-by-digit `amountCents` buildup + backspace), Request/Pay/Split pill toggle, avatar-based recent-contacts row, and a stepper (2–8 people) for Split mode's participant count in place of the old raw number `TextInput`. Split math itself still goes through the existing, already-verified `splitEvenly()` (the design mock's own math ignored the creator's-share remainder; that simplification was not carried over — correctness came from the real function, only the *input UI* came from the design).
- `RequestScreen` (incoming request) — avatar, destination-address card with the existing anti-impersonation warning, decline/pay actions. Added an explicit "declined" confirmation sub-state (design shows one; the old code just did an immediate `goBack()`), and folded the existing `alreadyPaid`/`expired` checks into the same message+Done pattern the design uses for `declined`, since the design's own mock never modeled those two real cases at all.
- `ShareRequestScreen` (share/QR) — unchanged real logic (QR code, copy, share, live `isRequestMarkedPaid` paid badge), fully restyled.
- `ShareBillScreen` (split-bill) — **deliberately dropped one piece of the design's mock interactivity**: its "Mark paid" button let you manually flip any participant's paid flag with no real check behind it. Real paid status can only come from the on-chain `isRequestMarkedPaid()` check (already used elsewhere in this exact file) — a manual override button would let the UI show "paid" for a share nobody actually paid, which is precisely the kind of mockup behavior removing-mock-data means removing. Kept the real "Copy" and "Remind" actions instead.
- `ClaimUsernameScreen` — restyled with the design's exact copy and availability-state border coloring; all availability-check/claim logic unchanged.
- `QrCode.tsx` — one cosmetic radius tweak (12→14px) to match the design's QR container exactly; otherwise unchanged (already pure-JS, no native dependency, from Phase 4).
- New `src/components/Avatar.tsx` (colored initials circle, used everywhere the design shows one) and `src/lib/ToastProvider.tsx` (a bottom pill toast with fade+slide animation, replacing `Alert.alert()` for low-stakes confirmations like "Link copied"/"Reminder sent" — the design explicitly specs a toast component for exactly this, and a modal `Alert` didn't fit its tone).
- `src/lib/navigation.ts`: added an optional `mode` param to the `New` route so Home's "Pay someone" / "Request money" buttons can jump straight into the right composer pill, matching the design's two distinct entry buttons (previously both routes landed on the same default `request` pill).

**Two real bugs found live-testing on the emulator after the rebuild, both fixed same-session:**
1. **Every custom in-screen header sat flush against the status bar/notch, un-tappable.** Root cause: hiding React Navigation's native header (`headerShown: false`, done to match the design's own custom back-button style instead of a native header bar) also removed the automatic top safe-area inset that native header used to provide — nothing replaced it. Fixed by wrapping every screen's root in `SafeAreaView` from `react-native-safe-area-context` (already an existing dependency, just never used directly before) and adding a top-level `SafeAreaProvider` in `App.tsx`. JS-only fix, no rebuild needed — a plain Metro reload picked it up.
2. **Screen transitions had no motion** — used `animation: 'fade'` in the stack navigator's `screenOptions`, a flat cross-fade with none of the design's "kivoIn" keyframe (fade + slight upward slide, applied to every screen in the shared design). Switched to native-stack's built-in `'fade_from_bottom'` preset, which is the native equivalent of that exact motion — a platform-native transition rather than an attempt to hand-replicate a CSS keyframe, and confirmed a closer match to what the design actually specified. Also JS-only, no rebuild.

**User confirmed working** after these two fixes — redesign is complete and live-verified on-device, both fresh (empty-state) and against the flows exercised earlier this session (composer, Home, recent contacts).

### 2026-09-21 — Session 3 continued: Home's request list generalized into a real Activity feed

User's ask, after seeing the redesigned Home screen live: rename the "Requests" section to something broader since it visually reads like it should show payments and splits too — but flagged for feedback first rather than just renaming.

**The real gap this surfaced, not just a label problem:** Home's list only ever showed requests *you'd created* (`getSentRequests()`). Payments you *made* (Pay mode, or paying someone else's incoming request) were never recorded anywhere as a history item — only added to the recent-contacts autofill list. Renaming the label to "Activity" without fixing that would have made the label a lie the first time someone tested Pay mode and didn't see it show up. Explained this to the user before writing anything; agreed to build both pieces together.

**Added `SentPayment` to `localStore.ts`** (`addSentPayment`/`getSentPayments`) — the outgoing-money mirror of the existing `SentRequest`. Wired at both places money actually leaves the wallet:
- `NewRequestScreen.tsx`'s Pay-mode success path (alongside the existing `recordRecentContact` call).
- `RequestScreen.tsx`'s incoming-request `pay()` success path — paying someone else's request is just as much an outgoing payment as using the composer's Pay mode, and would have been a second silent gap in the feed if left out.

**New `src/lib/activity.ts`** — a single `loadActivity(connection)` shared by both Home and the new history screen, so the merge-and-paid-check logic exists in exactly one place instead of being duplicated per screen (previously Home computed this inline). Combines sent requests (grouped into standalone vs. bill, same logic Home already had), sent payments, and returns one array sorted newest-first. Paid status for requests/bills is still a real `isRequestMarkedPaid()` on-chain check; payments need none, since they're only ever recorded after their own transaction already confirmed.

**New `src/components/ActivityRow.tsx`** — the single row renderer for all three item kinds (request/bill/payment), extracted out of `HomeScreen` so it isn't duplicated between Home's preview and the new full-history screen.
